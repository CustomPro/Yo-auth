...
validate.user = (user, password) => {
    ...
    if (!utils.compareStringWithHash(password, user.password)) {
...
validate.client = (client, clientSecret) => {
    ...
    if (!utils.compareStringWithHash(clientSecret, client.clientSecret)) {
...
validate.token = (token, accessToken) => {
    ...
    if (token.userId != null) {
        return db.users.findOne({ _id: token.userId})
            .then(user => validate.userExists(user))
            .then(user => user);
    }
    // token is a client token
    return db.clients.findOne({_id: token.clientId})
...
validate.refreshToken = (token, refreshToken, client) => {
    ...
    if (client.id !== token.clientId.toString()) {
        validate.logAndThrow('RefreshToken clientIs does not match client id given');
    }
    return refreshToken;
};
...
validate.authCode = (code, authCode, client, redirectURI) => {
    ...
    if (client.id !== authCode.clientId.toString()) {
...
validate.generateRefreshToken = ({userId, clientId, scope}) => {
    ...
    return db.refreshTokens.addToken({token: refreshToken, userId, clientId, scope})
        .then(() => refreshToken);
};
...
validate.generateToken = ({userId, clientId, scope}) => {
    ...
    return db.accessTokens.addToken({token, expirationDate, userId, clientId, scope})
        .then(() => token);
};
