...
exports.info = (req, res) =>
  ...
  .then(() => db.accessTokens.findToken(req.query.access_token))
  .then(token => validate.tokenExistsForHttp(token))
  .then(token =>
    db.clients.findOne({_id: token.clientId.toString()})
...
exports.revoke = (req, res) =>
  ...
  .then(() => db.accessTokens.removeToken(req.query.token))
  .then((token) => {
    if (token == null) {
      return db.refreshTokens.removeToken(req.query.token);
    }
    return token;
  })
