...
server.grant(oauth2orize.grant.code((client, redirectURI, user, ares, done) => {
    ...
    db.authorizationCodes.generateCode({
        code, redirectURI, 'clientId': client.id, 'userId': user.id, 'clientScope': client.scope
    })
        .then(() => done(null, code))
        .catch(err => done(err));
}));
...
server.grant(oauth2orize.grant.token((client, user, ares, done) => {
    ...
    db.accessTokens.addToken({token, expiration, userId: user.id, clientId: client.id, scope: client.scope})
...
server.exchange(oauth2orize.exchange.code((client, code, redirectURI, done) => {
    db.authorizationCodes.removeCode(code)
...
server.exchange(oauth2orize.exchange.clientCredentials((client, scope, done) => {
    ...
    db.accessTokens.addToken({token, expiration, clientId: client.id, scope})
...
server.exchange(oauth2orize.exchange.refreshToken((client, refreshToken, scope, done) => {
    ...
    db.refreshTokens.findToken(refreshToken)
        .then(foundRefreshToken => {
            const token = validate.refreshToken(foundRefreshToken, refreshToken, client);
...
exports.authorization = [
    ...
    server.authorization((clientId, redirectURI, scope, done) => {
        ...
        db.clients.findOne({clientId: req.query.client_id})
            .then((client) => {
